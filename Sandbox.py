...
import numpy as np
from PGM_functions import Factor, Gibbs_sampling, transform_dataset
...
# var_A = np.array([0])
# card_A = np.array([3])
# values_A = np.log(np.array([100, 50, 1]))

# var_B = np.array([1])
# card_B = np.array([3])
# values_B = np.log(np.array([50, 50, 50]))

# var_C = np.array([2])
# card_C = np.array([3])
# values_C = np.log(np.array([20,1,100]))

# var_AB = np.array([0, 1])
# card_AB = np.array([3, 3])
# values_AB = np.log(np.array([100,20,20,20,100,20,20,20,100]))

# var_AC = np.array([0, 2])
# card_AC = np.array([3, 3])
# values_AC = np.log(np.array([10,50,50,50,10,50,50,50,100]))
...
for x_id in range(num_variables):
	...
	F_C_Net = f_c_softmax_Group_LASSO(name, dim_classifier, cardinality, alpha=0.01, save_dir=save_dir)
...
for x_id in range(num_variables):
	...
	# Node factor
	new_Factor = Factor(np.array([x_id]), np.array([cardinality[x_id]]), node_factors[x_id])
	...
	for y_id in rest_variables:
		...
		if y_id > x_id:
			...
			if np.linalg.norm(c_factor_values) > 1E-6:
				...
				new_Factor = Factor(np.array([x_id, y_id]), np.array([cardinality[x_id], cardinality[y_id]]), c_factor_values)
				factor_list.append(new_Factor)
