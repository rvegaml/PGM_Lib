...
class General_Model_Group_LASSO():
	def __init__():
		dim_in_class = dim_model[0]
		dim_out_class = dim_model[1]
		...
		with tf.Session() as sess:
			...
			for i in range(num_iter):
				...
				if i % 100 == 0:
					...
					c_reg = self.regularization.eval(feed_dict={
						self.X:X, self.Y:Y
						})
					print(c_reg)
					print('step %d, current loss %g' % (i, c_loss))
	...
	def compute_regularization_term(self, W, cardinality, weights):

		reg = 0
		start_index = 0

		for element, weight in zip(cardinality, weights):
			end_index = start_index + element

			subset = W[start_index:end_index, :]

			reg+=(tf.norm(subset)*weight)
			start_index = end_index

		return reg


class Multiclass_Logistic_Regression_Group_LASSO(General_Model_Group_LASSO):

	def __init__(self, name, dim_model, cardinality, weights, alpha=10, save_dir='./trained_variables_Muliclass_LogReg'):
		dim_in_class = dim_model[0]
		dim_out_class = dim_model[1]
...
class Linear_Regression_Group_LASSO(General_Model_Group_LASSO):

	def __init__(self, name, dim_model, cardinality, weights, alpha=10, save_dir='./trained_variables_LinearReg'):
		dim_in_class = dim_model[0]
		dim_out_class = dim_model[1]
		...
		# Create the network of the first transformer
		# Layer 1
		self.W_H1, self.b_H1, z_hat, _ = fully_connected(name, 'H_L1', self.X, dim_in_class,
